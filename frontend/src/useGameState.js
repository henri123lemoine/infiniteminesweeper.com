...
export const useGameState = () => {
  ...
  const floodFillReveal = useCallback(async (startWorldX, startWorldY) => {
    ...
    while (queue.length > 0) {
      ...
      toReveal.add({
        worldX, worldY, chunkX, chunkY, localX, localY,
        adjacentMines, cellKey,
      });
  ...
  const handleCellClick = useCallback(async (worldX, worldY, isRightClick = false) => {
    ...
    if (isRightClick) {
      ...
      ws.send(encodeMsg(PB.Msg.create({
    ...
    if (revealedCell && !revealedCell.isMine && revealedCell.adjacentMines > 0) {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          const { chunkX: nChunkX, chunkY: nChunkY, localX: nLocalX, localY: nLocalY } =
          ...
          if (flaggedCellsRef.current.has(neighborFlagKey) ||
    ...
    if (isMine(seed, localX, localY)) {
      ...
      ws.send(encodeMsg(PB.Msg.create({
        reveal: { chunkId: { X: chunkX, Y: chunkY }, x: localX, y: localY },
      })));
      return;
    }
    ...
    if (adjacent === 0) {
      ...
      ws.send(encodeMsg(PB.Msg.create({
        reveal: { chunkId: { X: chunkX, Y: chunkY }, x: localX, y: localY, flow: true },
      })));
    } else {
      const optimisticReveal = {
        chunkId: { X: chunkX, Y: chunkY },
        x: localX, y: localY, playerId: -1,
        isMine: false, adjacentMines: adjacent,
      };
  ...
  }, [ws, connected, ensureChunkSubscription, countAdjacentMines, floodFillReveal]);

  const connectWs = useCallback((nameInput, playerColor) => {
    ...
    websocket.onmessage = (event) => {
      ...
      } else if (data.chunkId && typeof data.x === "number" && typeof data.y === "number") {
        if (data.color) {
          // Flag broadcast
  ...
  const sendViewUpdate = useCallback((viewX, viewY) => {
    if (ws && connected) {
      ws.send(encodeMsg(PB.Msg.create({
        viewUpdate: { viewX: Math.floor(viewX), viewY: Math.floor(viewY) },
      })));
    }
  }, [ws, connected]);
