...
func TestRightClickFlaggingAndPenalty(t *testing.T) {
	...
	if !ok {
		t.Fatalf("failed to find mine and safe cell")
	}
	...
	after := s.scores[1]
	if after <= before {
		t.Fatalf("expected positive score change on correct flag: %d -> %d", before, after)
	}
	if !s.isCellFlagged(cid, mine) {
		t.Fatalf("mine not flagged")
	}
	...
	after2 := s.scores[1]
	if after2 >= before2 {
		t.Fatalf("expected penalty on wrong flag: %d -> %d", before2, after2)
	}
	if !s.isCellRevealed(cid, safe) {
		t.Fatalf("safe cell not revealed after wrong-flag flow")
	}
}
...
func TestChordValidationRevealsNeighbors(t *testing.T) {
	...
	if !found {
		t.Skip("no suitable numbered cell found in chunk")
	}

	// Reveal the center first (as chord requires revealed center)
	s.handleReveal(1, 10, cid, center, false, false)
	if !s.isCellRevealed(cid, center) {
		t.Fatalf("center not revealed")
	}
	...
	if !revealedAny {
		t.Fatalf("chord did not reveal any safe neighbor")
	}
}
