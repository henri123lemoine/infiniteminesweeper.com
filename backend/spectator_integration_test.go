...
func TestFSMStateTransitions(t *testing.T) {
	...
	if err := writePB(c1, reveal); err != nil {
		t.Fatalf("write reveal as player: %v", err)
	}
...
func TestNameValidation(t *testing.T) {
	...
	if err := writePB(c1, join1); err != nil {
	...
	for time.Now().Before(deadline) {
		m, err := readPB(c1, 2*time.Second)
		if err != nil {
			t.Fatalf("read join1 response: %v", err)
		}
	...
	if err := writePB(c2, join2); err != nil {
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			t.Fatalf("read join2 response: %v", err)
		}
		if ack := m.GetJoinAck(); ack != nil {
			if ack.Ok {
