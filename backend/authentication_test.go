...
func TestAuthentication_NewAndReuseSessionToken(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("read joinAck1: %v", err)
	}
	j1 := decodePBMsg(t, j1bytes).GetJoinAck()
	if j1 == nil {
		t.Fatalf("expected JoinAck message")
	}
	if j1.SessionToken == "" {
		t.Fatalf("server did not issue a session token")
	}
	if got, want := j1.Name, "Alice"; got != want {
		t.Fatalf("name=%q want %q", got, want)
	}
	if j1.Score != 0 {
		t.Fatalf("score=%d want 0", j1.Score)
	}
	...
	if err != nil {
		t.Fatalf("read joinAck2: %v", err)
	}
	j2 := decodePBMsg(t, j2bytes).GetJoinAck()
	if j2 == nil {
		t.Fatalf("expected JoinAck2")
	}
	if j2.SessionToken != token {
		t.Fatalf("token changed on reuse")
	}
	...
	if len(srv.sessionTokens) == 0 {
		t.Fatalf("server did not persist session token")
	}
}
...
func TestFirstJoinUsernameTaken(t *testing.T) {
	_, wsURL, cleanup := startTestServer(t)
	defer cleanup()

	// First player takes the username "TestUser"
	c1, _, err := websocket.DefaultDialer.Dial(wsURL, nil)
	if err != nil {
		t.Fatalf("dial1: %v", err)
	}
	...
	join1 := &pb.Msg{Payload: &pb.Msg_Join{Join: &pb.Join{
		SessionToken: "", Name: "TestUser", FlagID: 1,
	}}}
	if err := c1.WriteMessage(websocket.BinaryMessage, mustProto(join1)); err != nil {
		t.Fatalf("write join1: %v", err)
	}
	...
	if !j1.Ok {
		t.Fatalf("first join should succeed, got error: %s", j1.Error)
	}
	if got, want := j1.Name, "TestUser"; got != want {
		t.Fatalf("name=%q want %q", got, want)
	}

	// Second player tries to use the same username (should fail)
	c2, _, err := websocket.DefaultDialer.Dial(wsURL, nil)
	if err != nil {
		t.Fatalf("dial2: %v", err)
	}
	...
	join2 := &pb.Msg{Payload: &pb.Msg_Join{Join: &pb.Join{
		SessionToken: "", Name: "TestUser", FlagID: 2,
	}}}
	if err := c2.WriteMessage(websocket.BinaryMessage, mustProto(join2)); err != nil {
		t.Fatalf("write join2: %v", err)
	}
	...
	// This is the key assertion: the join should be rejected
	if j2.Ok {
		t.Fatalf("second join with duplicate username should fail, but it succeeded")
	}
	if got, want := j2.Error, "Username already taken"; got != want {
