...
func TestWALFlushToDisk(t *testing.T) {
    ...
    for scanner.Scan() {
        line := scanner.Text()
        if strings.Contains(line, `"type":"reveal"`) { types["reveal"] = true }
        if strings.Contains(line, `"type":"flag"`) { types["flag"] = true }
        if strings.Contains(line, `"type":"score_update"`) { types["score_update"] = true }
    }
    if len(types) == 0 {
