...
func TestHandleHotspot(t *testing.T) {
    ...
    var resp struct{ X, Y int64; Count int }
...
func TestHandleLeaderboardHTTP(t *testing.T) {
    ...
    var out []struct{ Name string; Score int32; FlagID uint32 }
    if err := json.Unmarshal(rr.Body.Bytes(), &out); err != nil {
        t.Fatalf("json: %v", err)
    }
    if len(out) != 2 { t.Fatalf("len=%d want 2", len(out)) }
    if out[0].Name != "B" || out[0].Score != 300 { t.Fatalf("first=%+v", out[0]) }
    // Duplicate name "A" should keep the higher of 100 and 200 => 200
    if out[1].Name != "A" || out[1].Score != 200 { t.Fatalf("second=%+v", out[1]) }
}
