...
def graft_image(background_path: str, overlay_path: str, x: int, y: int, output_path: str) -> None:
    # Load background and overlay (RGBA to preserve transparency)
    try:
        bg = Image.open(background_path).convert("RGBA")
    except IOError as e:
        print(f"Error: cannot open background file {background_path}: {e}", file=sys.stderr)
        sys.exit(1)
    try:
    ...
    # Check bounds
    bg_w, bg_h = bg.size
    if not (0 <= x-1 < bg_w and 0 <= y-1 < bg_h and x+ov_w <= bg_w and y+ov_h <= bg_h):
...
def parse_args():
    ...
    p.add_argument("--background", "-b", required=True,
                   help="32×32 background PNG")
    p.add_argument("--overlay", "-i", required=True,
                   help="25×18 overlay PNG")
    p.add_argument("--output", "-o", required=True,
                   help="Where to write the 32×32 output PNG")
    p.add_argument("--x",      type=int, default=6,
                   help="Horizontal offset (default: 6)")
    p.add_argument("--y",      type=int, default=7,
                   help="Vertical offset (default: 7)")
    return p.parse_args()
